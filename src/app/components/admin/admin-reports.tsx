...
function generatePDF(
  ...
  doc.text("POS Batto", 14, 22);

  // Subtitle
  doc.setFont("helvetica", "normal");
  doc.setFontSize(10);
  doc.setTextColor(100, 116, 139); // slate-500
  doc.text("Smart Restaurant Management System", 14, 28);
  ...
  doc.text(`Period: ${dateRange}`, 196, 28, { align: "right" });
  doc.text(`Printed: ${new Date().toLocaleString()}`, 196, 33, { align: "right" });
  ...
  if (summaryLines.length > 0) {
    ...
    doc.roundedRect(14, 50, 182, 10 + (summaryLines.length * 7), 3, 3, "F");
    ...
    summaryLines.forEach((line) => {
      // Bold the key if it exists
      if (line.includes(":")) {
        const [k, v] = line.split(":");
        doc.setFont("helvetica", "bold");
        doc.text(`${k}:`, 20, y);
        doc.setFont("helvetica", "normal");
        doc.text(v, 20 + doc.getTextWidth(`${k}: `), y);
      } else {
        doc.text(line, 20, y);
      }
      y += 7;
    ...
    startY = 50 + 10 + (summaryLines.length * 7) + 10;
  }
  ...
  autoTable(doc, {
    ...
    didDrawPage: function (data) {
      ...
      doc.text(
        `Page ${data.pageNumber} of ${pageCount} - Generated by POS Batto`,
        105,
        287,
  ...
  doc.save(`POS_Batto_${title.replace(/\s+/g, "_")}_${new Date().getTime()}.pdf`);
}
